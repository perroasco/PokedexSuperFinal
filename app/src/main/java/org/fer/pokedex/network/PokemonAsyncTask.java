...
import static org.fer.pokedex.network.NetworkUtils.createUrl;
import static org.fer.pokedex.network.NetworkUtils.makeHttpRequest;
 ...
 public class PokemonAsyncTask extends AsyncTask<Void, Void, List<Pokemon>> {
    ...
    protected List<Pokemon> doInBackground(Void... voids) {
        ...
        URL url = createUrl("https://pokeapi.co/api/v2/pokemon?limit=135&offset=251");
        String jsonResponse = "";
        try {
            jsonResponse = makeHttpRequest(url);
            return pokemonlist(jsonResponse);
        } catch (IOException e) {
    ...
    protected void onPostExecute(List<Pokemon> pokemonList) {
     ...
     private List<Pokemon> pokemonlist(String jsonStr) {
         try {
